...
extension RoutingController: SessionDelegate {
    ...
    private func showLoginScreen() {
        ...
        let proposal = VisitProposal(url: Api.Path.login, options: VisitOptions(), properties: properties)
          ...
          case .modal:
              ...
              if let presentedViewController = presentedViewController {
                  presentedViewController.dismiss( animated: true, completion: {
                      [unowned self] in self.present(modalNavController, animated: true)
                  })
              }
              else {
    ...
    private func visit(_ vc: UIViewController, options: VisitOptions, presentationType: PresentationType) {
